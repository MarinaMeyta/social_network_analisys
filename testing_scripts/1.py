...
api = tweepy.API(auth)
results = api.search(q = "#kitty", count = 100)
con = sqlite3.connect('test.db')
cur = con.cursor()

cur.execute('CREATE TABLE IF NOT EXISTS tweets (tweet_id integer PRIMARY KEY NOT NULL, tweet_text VARCHAR(140),created_at text,user_name text,lang text,time_zone text,location text,numb_tweets text,numb_followers text )')
con.commit()
...
for result in results:
	...
	if (cur.fetchone() == None):
		...
		cur.execute("insert into tweets (tweet_id, tweet_text,created_at,user_name,lang,time_zone,location,numb_tweets,numb_followers) values(?,?,?,?,?,?,?,?,?)",(result.id_str,result.text,str(result.created_at),result.user.name,result.user.lang, str(result.user.time_zone),result.user.location, str(result.user.statuses_count),str(result.user.followers_count)))
		con.commit()
